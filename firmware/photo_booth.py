...
# -- button: A (GPIO5) or B (GPIO11), exposed through case --
# Camera bus noise causes spurious HIGH pulses on these pins.
# We require sustained HIGH over 5 consecutive reads (200ms) to confirm a real press.
_btn = Pin(5, Pin.IN, Pin.PULL_DOWN)


def button_pressed() -> bool:
    """Return True only if button held HIGH for 5 consecutive reads over 200ms."""
    if _btn.value() == 0:
        return False
    for _ in range(4):
        utime.sleep_ms(50)
